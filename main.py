...
async def help(ctx, cat=None):
	...
	if cat=='tools':
		...
		if await check(ctx):
			await ctx.message.edit(content=f'**__Selfbot by LALOL__\n{update}\n:skull:`{pref}nuke` - Уничтожение сервера\n:shushing_face:`{pref}silentnuke [ID Сервера] [Сообщение]` - Уничтожение сервера с обходом ВСЕХ анти-краш ботов +нельзя определить кто уничтожил сервер\n:smiling_imp:`{pref}spamchannels [Имя]` - Спам каналами\n:jack_o_lantern:`{pref}spamroles [Имя]` - Спам ролями\n:cold_face:`{pref}spamwebhooks [Сообщение]` - Спам вебхуками\n:clown:`{pref}deleteall` - Удаление всего\n\n`{pref}deletechannels` - Удаляет каналы\n`{pref}deleteroles` - Удаляет роли\n`{pref}deleteemojis` - Удаляет эмодзи**\n**Команды краша из селфбота Артема Бая\n:skull_crossbones:`>fashcrash` - быстрый краш\n:skull_crossbones:`>lavan` - краш с обходом анти-краш бота Лаван\n:skull_crossbones:`>chls` - создание и удаление каналов\n:skull_crossbones:`>rls` - создание и удалиние ролей\n:skull_crossbones:`>dlc` - удаление каналов0\n:skull_crossbones:`>dlr` - удаление всех ролей\n:skull_crossbones:`>rnc` - переименовать каналы\n:skull_crossbones:`>rnr` - переименование ролей\n:skull_crossbones:`>spamchannels` - спам рандом каналами\n:skull_crossbones:`>spamroles` - спам рандом ролями\n:skull_crossbones:`>banall` - забанить всех НЕ через реквесты\n:skull_crossbones:`>ban_all` - забанить всех ЧЕРЕЗ реквесты\n:skull_crossbones:`>spamemojis` - спам эмодзями\n:skull_crossbones:`>randomchannels` - быстрое создание рандом каналов\n:skull_crossbones:`>delcategories` - удалить категории\n:skull_crossbones:**')
	else:
